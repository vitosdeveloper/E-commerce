...
export function GlobalProvider({ children }) {
  ...
  const jwtCheck = async () => {
    const response = await Axios.post(
      globalContextState.serverUrl + '/checkJwt',
      { jwt }
    );
  ...
  useEffect(() => {
    const asyncInsideUseEffect = async () => {
      ...
      const dataEmbaralhada = data.sort(() => {
  ...
  useEffect(() => {
    const atualizarComprasDoUser = async () => {
      ...
      if (dataToVerify.userId.length > 0 && dataToVerify.jwt.length > 0) {
        const checkJwtAndGetUserHist = await Axios.post(
          globalContextState.serverUrl + '/alimentarHistorico',
          { dataToVerify }
        );
