...
function Carrinho() {
      ...
      carrinhoItens.forEach((item) => {
        if (item._id === itemId) {
          setCarrinhoItens((lastValues) => {
            ...
            return [
              ...carrinhoFiltrado,
              { _id: itemId, quantidade: itemQuantidade, preco: precoDoItem },
            ];
          });
        }
      });
  ...
  async function formularioDaCompra() {
    ...
    const response = await Axios.post(serverUrl + '/efetuarCompra', {
      formulario,
    });
    if (response.data.status === 'success') {
      ...
      document.querySelector('.divResponse').innerText =
        'Algum dos itens está fora de estoque!';
      setTimeout(() => {
      ...
      <div className='favoritados'>
        ...
        {carrinhoItens.length > 0 ? (
          itensDaLoja.map((item, index) => {
            return carrinhoItens.map((carItem) => {
                  ...
                  <Link className='linkLindo' to={'/' + item._id}>
                    <div className='item'>
                      ...
                      <div className='descri fromCar'>
                        ...
                        <h5 className='price'>{'R$ ' + item.productPrice}</h5>
                      </div>
                    </div>
                  </Link>
                </div>
        ...
        <div
          className='precoTotal'
          style={carrinhoItens.length === 1 ? { marginTop: '60px' } : {}}
        >
          {carrinhoItens.length > 0 ? (
            <h3>
              Preço total: <br />
          ...
          {carrinhoItens.length === 0 ? null : isLoggedIn === false ? (
      ...
      {confirmarCompra ? (
        <div className='confirmarCompra'>
          <div>
            ...
            <Link to='/profile'>
              <button className='trocarEndereço'>Trocar meu endereço</button>
            </Link>
          ...
          <div className='divResponse'>
            {precoTotal !== 0 ? (
              <button onClick={formularioDaCompra} className='finalizarCompra'>
                <h2>Efetuar compra</h2>
              </button>
            ) : (
              <button className='finalizarCompra'>
                <h2 style={{ textAlign: 'center' }}>Item fora de estoque</h2>
              </button>
            )}
          </div>
        </div>
