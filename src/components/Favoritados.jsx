...
function Favoritados() {
  ...
  function favToCart(e) {
    ...
    itensDaLoja.forEach((itemZ) => {
      return itemZ._id === itemId ? (pricePerItem = itemZ.productPrice) : null;
    });
      ...
      <div className='favoritados'>
        ...
        {favoritos.length >= 1 ? (
          itensDaLoja.map((item, index) => {
            return favoritos.includes(item._id) ? (
              <div key={index} className='listaDeItens'>
                <div className='favButtons'>
                  <button
                    name={item._id}
                    onClick={favDelete}
                    className='favExcluir favButComum'
                  >
          ...
          <div className='ifNotLogged'>
            ...
            <h4>
              Aproveite e favorite seus itens preferidos através da{' '}
